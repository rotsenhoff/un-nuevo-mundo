...
def max_recycling(crates):
    """Returns the index with the largest value in the list and the number of crates for that
    house.
...
def min_recycling(crates):
    """Returns the index with the smallest value in the list and the number of crates for that
    house.
    Raises ValueError if the list is None or empty."""
...
def total_crates(crates):
    """Returns the total of all the values in the crates list."""
...
def get_crate_quantities(houses):
    """Ask usr for number of crates for each house"""
...
def positive_int_input(question):
    """Validate user enters a positive integer"""
...
def main():
    ...
    print('The house(s) with the most recycling is/are {}'.format(maximums.houses))

    print('The min number of crates from any house is {}'.format(minimums.crates))
    print('The house(s) with the least recycling is/are {}'.format(minimums.houses))
